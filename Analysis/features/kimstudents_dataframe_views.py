...
def ratio_of_phenotypes(df, generalized=True):
    ...
    #             va='center', xytext=(0, 10), textcoords='offset points')
    # plt.savefig(os.path.join(FIGURE_DIR, f'{type_}ratio.pdf'))
    # plt.close(fig)
...
def penetrance(df):
    ...
    plt.legend(ax, [vf.HPO_ABBREVIATIONS[v] for v in cdf.columns])
...
def phenotype_correlation_ratio(df):
    ...
    # normalizing so that diagonals = 1; could probably do better
...
def create_figures(df, analysis_type):
    ...
    for fn in fns:
        ...
        plt.savefig(os.path.join(fig_path, f'{fig_name}.pdf'))