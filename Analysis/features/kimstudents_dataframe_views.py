...
def regions_alpha_beta(df):
    ...
    pheno_domains = pheno_domains.rename_axis("Phenotype", axis=0).rename_axis("Region", axis=1).rename(lambda x: x.split(".")[1])
...
def regions(df):
    ...
    for row in COMPUTED_COLUMNS["generalized_phenotype"]:
        ...
        pheno_alpha.loc[row, "⍺-Domain"] = len(df_phen[(df_phen['region.⍺-Domain'] >= 1) & (df_phen['region.ElonginB_ElonginC_binding'].isna())])

        pheno_alpha.loc[row, "ElonginB_ElonginC_binding"] = len(df_phen[(df_phen['region.⍺-Domain'] >= 1) & (df_phen['region.ElonginB_ElonginC_binding'] >= 1)])

        pheno_beta.loc[row, "β-Domain"] = len(df_phen[(df_phen['region.β-Domain'] >= 1) & (df_phen['region.HIF1_alpha_binding'].isna())])

        pheno_beta.loc[row, "HIF1_alpha_binding"] = len(df_phen[(df_phen['region.β-Domain'] >= 1) & (df_phen['region.HIF1_alpha_binding'] >= 1)])

        pheno_cds.loc[row, "Outside of ⍺-Domain and β-Domain"] = len(df_phen[(df_phen['region.Outside of ⍺-Domain and β-Domain'] >= 1) & (df_phen['region.GXEEX8'].isna())])

        pheno_cds.loc[row, "GXEEX8"] = len(df_phen[(df_phen['region.Outside of ⍺-Domain and β-Domain'] >= 1) & (df_phen['region.GXEEX8'] >= 1)])

    all_phen_dfs = [pheno_alpha, pheno_beta, pheno_cds]
    for i in range(len(all_phen_dfs)):
        all_phen_dfs[i] = all_phen_dfs[i].rename_axis("Phenotype", axis=0).rename_axis("Region", axis=1).rename(lambda x: x.split(".")[1])
...
def domains_adjusted(df):
    ...
    pheno_domains = pheno_domains.rename_axis("Phenotype", axis=0).rename_axis("Domain", axis=1).rename(lambda x: x.split(".")[1])
    pheno_domains = pheno_domains.sort_index()
    ...
    pheno_domains["region.Outside of ⍺-Domain and β-Domain"] = pheno_domains["region.Outside of ⍺-Domain and β-Domain"] * (vf.CDS_LEN / (1 - vf.ALPHA_LEN + vf.BETA_LEN))
...
def mutant_type_counts(df):
    ...
    pheno_muttypes = pd.DataFrame(columns=COMPUTED_COLUMNS["generalized_mutant_type"], index=COMPUTED_COLUMNS["generalized_phenotype"])
...
def codon_histogram(df):
    ...
    for p in ax1.patches:
        ...
        # ax.annotate(str(p.get_height()), (p.get_x() * 1.005, p.get_height() * 1.005))
            plt.annotate(str(int(p.get_x() + p.get_width() / 2.)), (p.get_x() + p.get_width(), p.get_height()), ha='left',
...
def ratio_of_phenotypes(df, generalized=True):
    ...
    phens_ratio = phens_counts.rename_axis("Phenotype").rename("Share of Observations").rename(lambda x: x.split(".")[1])
...
def _phenotype_correlation(df):
    pheno_pheno = pd.DataFrame(columns=COMPUTED_COLUMNS["generalized_phenotype"], index=COMPUTED_COLUMNS["generalized_phenotype"])
...
def plot_cluster_property(df, figure_path, property_name, use_mean=False, save_csv=False, ratio_type='within'):
...
def create_cluster_summaries(df, analysis_type):
    fig_path = os.path.join(STATS_DIR, analysis_type, "cluster")
    if not os.path.isdir(fig_path):
        os.makedirs(fig_path)
    ...
    for prop in properties:
        ...
        plot_cluster_property(prop_df, figure_path=fig_path, property_name=prop, save_csv=True)

    prop = "age"
    prop_df = df.set_index("cluster_labels")[COMPUTED_COLUMNS[prop]]
    plot_cluster_property(prop_df, figure_path=fig_path, property_name=prop, use_mean=True)
...
def create_cluster_phenotype_summaries(df, analysis_type):
    df = df.set_index("cluster_labels")
    for phenotype in COMPUTED_COLUMNS["generalized_phenotype"]:
        fig_path = os.path.join(STATS_DIR, analysis_type, "cluster", phenotype)
        if not os.path.isdir(fig_path):
            os.makedirs(fig_path)
        ...
        for prop in properties:
            ...
            plot_cluster_property(prop_df, figure_path=fig_path, property_name=prop, ratio_type="across")




        prop_df = df_phen[phenotype].fillna(0)
        df_counts = prop_df.groupby("cluster_labels").sum()
